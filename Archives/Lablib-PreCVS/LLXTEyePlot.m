...
// Clear out any old events every so often
	...
	if (((++purgeCount % kPurgePeriod) == 0) && [data count] > 0) {
		limitTime = [[data lastObject] pointValue].y - durationS * 1000.0;
		for (index = 0; index < [data count]; index++) {
			if ([[data objectAtIndex:index] pointValue].y >= limitTime) {
				break;
			}
		}
		if (index > 0) {
			[data removeObjectsInRange:NSMakeRange(0, index)];
		}
	}

// Add the new event

    [data addObject:[NSValue valueWithPoint:eyePoint]];
	[eyeLock unlock];
}
...
// Draw the eye positions
    ...
    if ([data count] >= 2) {
        ...
        p = [scale scaledPoint:[[data objectAtIndex:0] pointValue]];
        [path moveToPoint:p];
        for (index = 1; index < [data count]; index++) {
            p = [scale scaledPoint:[[data objectAtIndex:index] pointValue]];
            [path lineToPoint:p];
        }
        [lineColor set];
        [path stroke];
