...
// If there is a delay before the first execution, delay here
    ...
    if (delayS > 0) {
        targetDate = [startDate addTimeInterval:delayS];
...
// If it is not yet time for the next execution, make the thread sleep
        ...
        targetDate = [startDate addTimeInterval:(delayS + (index + 1) * periodS)];
        if ([targetDate timeIntervalSinceNow] > 0) {
			[NSThread sleepUntilDate:targetDate];
...
// Check for index overflow
		...
		if (index == ULONG_MAX) {
			tempDate = [startDate addTimeInterval:index * periodS];
			[startDate release];
			startDate = tempDate;
			[startDate retain];
			index = 0;
