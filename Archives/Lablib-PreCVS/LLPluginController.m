...
int sortPluginsByName(id task1, id task2, void *context);
...
// For now we just make a list of all LLTaskPlugins available.
			...
			if ([pluginDesc.class isSubclassOfClass:[LLTaskPlugIn class]]) {
				if ([pluginDesc.class version] != kLLPluginVersion) {
					NSRunCriticalAlertPanel(@"LLPluginController: error loading plugin", 
						@"%@ has version %d, but current version is %d.  It will be not be used.", 
						@"OK", nil, nil, currPath, [pluginDesc.class version], 
						kLLPluginVersion);
				}
				else {
...
int sortPluginsByName(id desc1, id desc2, void *context)
{
	PluginDesc p1, p2;
