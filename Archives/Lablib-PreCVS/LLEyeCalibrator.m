...
// Set default calibration values, then try to read stored values
		...
		defaultsPath = [[NSBundle bundleForClass:[self class]] pathForResource:@"LLEyeCalibrator" ofType:@"plist"];
...
/*
 - (void)loadOffsets;
 {
	 ...
	 float halfCalOffsetDeg;				// half of the offset used for calibration, in degrees
	 float calOffsetDeg;
	 NSSize halfCalOffsetUnits;			// half of the offset used for calibration, in units
										//	float azimuthDeg, elevationDeg;
										//	NSSize originOffsetUnits;			// offset of origin, in units
	 
	 // Load the offset values.  The fixation azimuth and elevation offsets are added in here 
	 
	 calOffsetDeg = [taskDefaults floatForKey:LLFixCalOffsetDegKey];
	 halfCalOffsetUnits = [self unitSizeFromDegSize:NSMakeSize(calOffsetDeg, calOffsetDeg)];
	 halfCalOffsetUnits.width /= 2.0;
	 halfCalOffsetUnits.height /= 2.0;
	 halfCalOffsetDeg = [taskDefaults floatForKey:LLFixCalOffsetDegKey] / 2.0;
	 for (index = 0; index < kLLEyeCalibratorOffsets; index++) {
		 offsetDeg[index].x = ((index % 2) ? halfCalOffsetDeg : -halfCalOffsetDeg);
		 offsetDeg[index].y = ((index / 2) ? -halfCalOffsetDeg : halfCalOffsetDeg);
		 offsetUnits[index].x = ((index % 2) ? halfCalOffsetUnits.width : -halfCalOffsetUnits.width);
		 offsetUnits[index].y = ((index / 2) ? -halfCalOffsetUnits.height : halfCalOffsetUnits.height);
	 }
 }
 */
...
// Return the total offset, including both the calibration offset and the azimuth/elevation offset
...
- (NSPoint)offsetDeg {
	...
	if (offsetIndex < 0) {
		...
		return NSMakePoint(offsetDeg[offsetIndex].x + [taskDefaults floatForKey:LLFixCalAzimuthDegKey], 
					offsetDeg[offsetIndex].y + [taskDefaults floatForKey:LLFixCalElevationDegKey]);
	}
}
...
- (NSSize)unitSizeFromDegSize:(NSSize)sizeDeg {
	...
	return [degToUnits transformSize:sizeDeg];
}
