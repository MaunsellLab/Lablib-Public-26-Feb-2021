...
dialog, and NSUserDefaults.  the text field want to always be bound to instance variables in  LLVisualStimulus
(their owner), and sometimes we want the instance variables to be bound to NSUserDefaults.  But the NSUserDefaults is
optional, because we can't have all instances of an LLVisualStimulus bound to the same defaults. If the user 
specifies a prefix for the keys for NSUserDefaults, we activate binding to NSUserDefaults. (Note that the user
...
{	
	if ((self = [super init]) != nil) {
		...
		foreColor = [[NSColor colorWithCalibratedRed:1.0 green:1.0 blue:1.0 alpha:1.0] retain];
		backColor = [[NSColor colorWithCalibratedRed:0.5 green:0.5 blue:0.5 alpha:1.0] retain];
	}
	return self;
}
...
- (void)setBackColor:(NSColor *)newColor;
{
	...
	if (prefix != nil) {
		[[NSUserDefaults standardUserDefaults] 
				...
				forKey:[NSString stringWithFormat:[LLTextUtil capitalize:LLBackColorKey prefix:prefix]]];
	}
}
...
- (void)setForeColor:(NSColor *)newColor;
{
	...
	if (prefix != nil) {
		[[NSUserDefaults standardUserDefaults] 
				...
				forKey:[NSString stringWithFormat:[LLTextUtil capitalize:LLForeColorKey prefix:prefix]]];
	}
}
...
- (void)updateFloatDefault:(float)value key:(NSString *)key;
{
	if (prefix != nil && !setUnderway) {
		setUnderway = YES;
//		NSLog(@"Updating %@ to %f", 
//			[NSString stringWithFormat:[LLTextUtil capitalize:key prefix:prefix]], value);
		[[NSUserDefaults standardUserDefaults] setFloat:value 
				forKey:[NSString stringWithFormat:[LLTextUtil capitalize:key prefix:prefix]]];
		setUnderway = NO;
