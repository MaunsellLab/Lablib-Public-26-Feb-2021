...
NSString *behaviorWindowVisibleKey = @"Behavior Window Visible";
NSString *behaviorWindowZoomKey = @"Behavior Window Zoom";
...
// If settings have changed (number of stimulus levels, type of stim, etc.  we reset and redraw
		...
		[self reset:[NSData data] eventTime:[NSNumber numberWithLong:nil]]; 
	}
}
...
- (id) init {
    ...
    if ((self = [super initWithWindowNibName:@"BehaviorController"]) != Nil) {
        [self setWindowFrameAutosaveName:@"BehaviorController"];
		scaleFactor = 1.0;
...
- (void)windowDidBecomeKey:(NSNotification *)aNotification {
	...
	[[NSUserDefaults standardUserDefaults] setObject:[NSNumber numberWithBool:YES] 
                forKey:behaviorWindowVisibleKey];
}
...
// because setScaleFactor uses that
    ...
    defaultZoom = [[NSUserDefaults standardUserDefaults] integerForKey:behaviorWindowZoomKey];
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:behaviorWindowVisibleKey]) {
