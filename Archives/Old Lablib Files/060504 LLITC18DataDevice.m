...
{
//	NSLog(@"Samples: Currently 0x%+0x Enabling 0x%+0x", sampleChannels, [bitPattern unsignedLongValue]);
...
{
//	NSLog(@"Timestamps: Currently 0x%0x Enabling 0x%0x", timestampChannels, [bitPattern unsignedLongValue]);
...
// Allocate buffer to hold largest set of data, and then load instructions into the ITC

		pSamples = malloc(kMaxInstructions * sizeof(short));
		if (pSamples == nil) {
...
// Now the ITC is closed, and we have a valid pointer
	...
	NSLog(@"LLITC18DataDevice: succeeded initialize device %d using code %d",
					deviceNum, deviceNum | interfaceCodes[code]);
    [deviceLock unlock];
...
// Check for FIFO overflow
	...
	ITC18_GetFIFOOverflow(itc, &overflow);
	if (overflow != 0) {
		NSRunAlertPanel(@"LLITC18IODevice",  @"Fatal error: FIFO overflow", @"OK", nil, nil);
		exit(0);
	}

// Keep unloading complete cycles of samples one at a time while any are available

	ITC18_GetFIFOReadAvailable(itc, &available);						// available must be one more than a cycle, 
	while (available > numInstructions) {								//       so ITC doesn't go empty
		ITC18_ReadFIFO(itc, numInstructions, pSamples);					// read all the data
				...
				for (bitIndex = 0; bitIndex < kLLITC18DigitalBits; bitIndex++) { // check every bit
					...
					if (whichBit & timestampChannels) {					// enabled bit?
						...
						if (timestampActiveBits & whichBit) {			// was active
							...
							timestamp = sampleTimeS / timestampTickS[bitIndex];
