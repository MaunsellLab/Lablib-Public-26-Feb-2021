...
- (void)registerParameters:(LLParameter *)paramList {
    ...
    while (paramList != NULL) {
        if (parameters[paramList->name] != nil) {
        ...
        case kLong:
        case kUnsignedLong:
            value = @((int)(paramList->defaultValue).longParam);
            break;
        case kFloat:
            value = @((paramList->defaultValue).floatParam);
            break;
        case kShort:
        case kUnsignedShort:
        case kDouble:
        ...
        if (value != nil) {
            parameters[paramList->name] = value;
        }
        if (data != nil) {
            parameters[paramList->name] = data;
        }
        paramList++;
    }
