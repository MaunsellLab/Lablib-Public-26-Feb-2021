...
- (instancetype)init;
{
    if ((self =  [super initWithWindowNibName:@"LLSynthDataSettings"])) {
        self.windowFrameAutosaveName = @"LLSynthDataSettings";
...
- (void)loadValues {
    ...
    for (index = 0; index < kLLSynthDigitalBits; index++) {
        [spikeButtonBits cellAtRow:index column:0].state = ((bits & (0x1 << index)) > 0) ? NSOnState : NSOffState;
    }
}
