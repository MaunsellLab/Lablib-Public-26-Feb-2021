...
- (void)changeSize:(float)factor;
{
    switch (stimType) {
        case kBarStimulus:
            bar.lengthDeg = (bar.lengthDeg  * factor);
            break;
        case kGaborStimulus:
            gabor.radiusDeg = (gabor.radiusDeg  * factor);
            gabor.sigmaDeg = (gabor.sigmaDeg  * factor);
            break;
        case kDotsStimulus:
            dots.radiusDeg = (dots.radiusDeg  * factor);
            break;
        case kPlaidStimulus:
            plaid.radiusDeg = (plaid.radiusDeg  * factor);
            plaid.sigmaDeg = (plaid.sigmaDeg  * factor);
            break;
        default:
            break;
    }
...
- (void)changeWidth:(float)factor;
{
    switch (stimType) {
        case kBarStimulus:
            bar.widthDeg = (bar.widthDeg  * factor);
            break;
        case kGaborStimulus:
            gabor.spatialFreqCPD = (gabor.spatialFreqCPD / factor);
            break;
        case kDotsStimulus:
            dots.density = (dots.density * factor);
            [task.stimWindow lock];
            [dots makeMovie:5000];
            [task.stimWindow unlock];
            break;
        case kPlaidStimulus:
            plaid.spatialFreqCPD0 = plaid.spatialFreqCPD0 / factor;
            plaid.spatialFreqCPD1 = plaid.spatialFreqCPD1 / factor;
            break;
        default:
            break;
    }
...
- (void)rotate:(float)deltaDeg;
{
    ...
    switch (stimType) {
        case kGaborStimulus:
            gabor.directionDeg = (((long)(gabor.directionDeg / absDeltaDeg)) * absDeltaDeg + deltaDeg);
            break;
        case kBarStimulus:
            bar.directionDeg = (((long)(bar.directionDeg / absDeltaDeg)) * absDeltaDeg + deltaDeg);
            break;
        case kDotsStimulus:
            dots.directionDeg = (((long)(dots.directionDeg / absDeltaDeg)) * absDeltaDeg + deltaDeg);
            [task.stimWindow lock];
            [dots makeMovie:5000];
            [task.stimWindow unlock];
            break;
        case kPlaidStimulus:
            plaid.directionDeg0 = (((long)(plaid.directionDeg0 / absDeltaDeg)) * absDeltaDeg + deltaDeg);
            plaid.directionDeg1 = (((long)(plaid.directionDeg1 / absDeltaDeg)) * absDeltaDeg + deltaDeg);
            break;
    }
}
