...
NSString *MTCXTAutosaveKey = @"MTCXTAutosave";

@implementation MTCXTController
...
- (id)init {
    ...
    if ((self = [super initWithWindowNibName:@"MTCXTController"]) != nil) {
 		[self setShouldCascadeWindows:NO];
        [self setWindowFrameAutosaveName:MTCXTAutosaveKey];
...
// setting the maximum zoom size when the scale changes.
	...
	[[self window] setFrameUsingName:MTCXTAutosaveKey];			// Needed when opened a second time
...
- (void)stimulus:(NSData *)eventData eventTime:(NSNumber *)eventTime;
{
	...
	if ((stimDesc.attendLoc == 0 && stimDesc.type0 == kTargetStim) ||
		...
		[xtView eventName:[NSString stringWithFormat:@"Target %.0f%%", stimDesc.contrastPC]
											eventTime:eventTime];
	}
	else {
		[xtView eventName:[NSString stringWithFormat:@"%.0f%%", stimDesc.contrastPC]
											eventTime:eventTime];
	}
}
