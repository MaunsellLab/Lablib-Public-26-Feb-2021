...
NSString *TUNXTAutosaveKey = @"TUNXTAutosave";

@implementation TUNXTController
...
- (id)init {
    ...
    if ((self = [super initWithWindowNibName:@"TUNXTController"]) != nil) {
 		[self setShouldCascadeWindows:NO];
        [self setWindowFrameAutosaveName:TUNXTAutosaveKey];
...
// setting the maximum zoom size when the scale changes.
	...
	[[self window] setFrameUsingName:TUNXTAutosaveKey];			// Needed when opened a second time
...
- (void)stimulus:(NSData *)eventData eventTime:(NSNumber *)eventTime;
{
	...
	value = testParams.values[stimDesc.stimIndex];
	[xtView stimulusBarColor:[NSColor grayColor] eventTime:eventTime];
	[xtView eventName:[NSString stringWithFormat:@"%.*f", 
			[LLTextUtil precisionForValue:value significantDigits:1], value]
			eventTime:eventTime];
}
