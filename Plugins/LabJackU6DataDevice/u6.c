...
HANDLE openUSBConnection(int localID)
{
    ...
    numDevices = (uint32)LJUSB_GetDevCount(U6_PRODUCT_ID);
    if(numDevices == 0)
    {
...
long getCalibrationInfo(HANDLE hDevice, u6CalibrationInfo *caliInfo)
{
    ...
    sentRec = (int)LJUSB_BulkWrite(hDevice, U6_PIPE_EP1_OUT, sendBuffer, 26);
    if(sentRec < 26)
    {
        if(sentRec == 0)
            goto writeError0;
    ...
    sentRec = (int)LJUSB_BulkRead(hDevice, U6_PIPE_EP2_IN, recBuffer, 38);
    if(sentRec < 38)
    {
        if(sentRec == 0)
            goto readError0;
    ...
    for(i = 0; i < 10; i++)
    {
        ...
        sentRec = (int)LJUSB_BulkWrite(hDevice, U6_PIPE_EP1_OUT, sendBuffer, 8);
        if(sentRec < 8)
        {
            if(sentRec == 0)
                goto writeError0;
        ...
        sentRec = (int)LJUSB_BulkRead(hDevice, U6_PIPE_EP2_IN, recBuffer, 40);
        if(sentRec < 40)
        {
            if(sentRec == 0)
                goto readError0;
...
long getTdacCalibrationInfo(HANDLE hDevice, u6TdacCalibrationInfo *caliInfo, uint8 DIOAPinNum) {
    ...
    err = (int)I2C(hDevice, options, speedAdjust, sdaPinNum, sclPinNum, address, numByteToSend, numBytesToReceive, bytesCommand, &errorcode, ackArray, bytesResponse);
...
long I2C(HANDLE hDevice, uint8 I2COptions, uint8 SpeedAdjust, uint8 SDAPinNum, uint8 SCLPinNum, uint8 Address, uint8 NumI2CBytesToSend, uint8 NumI2CBytesToReceive, uint8 *I2CBytesCommand, uint8 *Errorcode, uint8 *AckArray, uint8 *I2CBytesResponse)
{
    ...
    *Errorcode = 0;
    ...
    sendChars = (int)LJUSB_BulkWrite(hDevice, U6_PIPE_EP1_OUT, sendBuff, sendSize);
    ...
    recChars = (int)LJUSB_BulkRead(hDevice, U6_PIPE_EP2_IN, recBuff, recSize);
    if(recChars < recSize)
    {
        if(recChars == 0)
...
long ehConfigIO(HANDLE hDevice, uint8 inWriteMask, uint8 inNumberTimersEnabled, uint8 inCounterEnable, uint8 inPinOffset, uint8 *outNumberTimersEnabled, uint8 *outCounterEnable, uint8 *outPinOffset)
{
    ...
    //Sending command to U6
    if( (sendChars = (int)LJUSB_BulkWrite(hDevice, U6_PIPE_EP1_OUT, sendBuff, 16)) < 16)
    {
        if(sendChars == 0)
    ...
    //Reading response from U6
    if( (recChars = (int)LJUSB_BulkRead(hDevice, U6_PIPE_EP2_IN, recBuff, 16)) < 16)
    {
        if(recChars == 0)
...
long ehConfigTimerClock(HANDLE hDevice, uint8 inTimerClockConfig, uint8 inTimerClockDivisor, uint8 *outTimerClockConfig, uint8 *outTimerClockDivisor)
{
    ...
    //Sending command to U6
    if( (sendChars = (int)LJUSB_BulkWrite(hDevice, U6_PIPE_EP1_OUT, sendBuff, 10)) < 10)
    {
        if(sendChars == 0)
    ...
    //Reading response from U6
    if( (recChars = (int)LJUSB_BulkRead(hDevice, U6_PIPE_EP2_IN, recBuff, 10)) < 10)
    {
        if(recChars == 0)
...
long ehFeedback(HANDLE hDevice, uint8 *inIOTypesDataBuff, long inIOTypesDataSize, uint8 *outErrorcode, uint8 *outErrorFrame, uint8 *outDataBuff, long outDataSize)
{
    ...
    if(((sendDWSize = (int)inIOTypesDataSize + 1)%2) != 0)
        sendDWSize++;
    if(((recDWSize = (int)outDataSize + 3)%2) != 0)
        recDWSize++;
    ...
    //Sending command to U6
    if( (sendChars = (int)LJUSB_BulkWrite(hDevice, U6_PIPE_EP1_OUT, sendBuff, (sendDWSize+commandBytes))) < sendDWSize+commandBytes)
    {
        if(sendChars == 0)
    ...
    //Reading response from U6
    if( (recChars = (int)LJUSB_BulkRead(hDevice, U6_PIPE_EP2_IN, recBuff, (commandBytes+recDWSize))) < commandBytes+recDWSize)
    {
        if(recChars == -1)
        {
