...
- (void)dealloc;
{
	if (eyelink_is_connected()) {
		set_offline_mode();					// place LabJackU6 tracker in off-line (idle) mode
...
// If the link to the LabJackU6 computer is not opening, check that the IP address for the Mac port that connects to t
// the LabJackU6 has been set to Manual and 100.1.1.2 and subnet mask 255.255.255.0
...
//		NSRunAlertPanel(@"LabJackU6DataDevice",  
//				@"Request to disable non-existent channel for device %@ (only %lu channels)",
...
//		NSRunAlertPanel(@"LabJackU6DataDevice", @"Requested sample period %ld of %lu for device %@",
